<template>
    <div id="commentCard">
        ...
        <div class="info-text">
            ...
            <div v-if="!Reply">
            <b-button v-on:click="showReply()" v-if="$store.state.loginStatus.authToken !== ''" style="width:100px;margin-left:5px;" pill class="mr-2" size="sm">Reply</b-button>
            <form ref="reply" v-if="$store.state.loginStatus.authToken !== ''" @submit.prevent="submitComment()" style="margin-bottom:15px;" class="hide">
            ...
            <comment-card @count="count" style="margin-top:25px;" v-for="comment in this.Comment.Replies" v-bind:key="comment.CommentID.Value" v-bind:Comment="comment" v-bind:Reply="true">
                <!-- <div id="sep" style="margin: 12.5px;" /> -->
            </comment-card>
        </div>
    </div>
</template>
